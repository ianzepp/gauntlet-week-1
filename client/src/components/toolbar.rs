...
pub fn Toolbar() -> impl IntoView {
    ...
    let is_board_route = move || location.pathname.get().starts_with("/app/board/");
    let show_share_dialog = move || {
        if show_share.get() {
            view! { <ShareDialog board=board on_cancel=on_share_cancel /> }.into_any()
        } else {
            ().into_any()
        }
    };
