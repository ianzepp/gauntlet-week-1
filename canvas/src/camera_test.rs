...
#[test]
fn camera_default_rotation_is_zero() {
    let cam = Camera::default();
...
fn screen_to_world_identity() {
    let cam = Camera::default();
    let world = cam.screen_to_world(Point::new(50.0, 75.0), origin());
    assert!(point_approx_eq(world, Point::new(50.0, 75.0)));
}

#[test]
fn screen_to_world_with_zoom() {
    let cam = Camera { pan_x: 0.0, pan_y: 0.0, zoom: 4.0, view_rotation_deg: 0.0 };
    let world = cam.screen_to_world(Point::new(40.0, 80.0), origin());
...
fn screen_to_world_with_pan() {
    let cam = Camera { pan_x: 100.0, pan_y: 50.0, zoom: 1.0, view_rotation_deg: 0.0 };
    let world = cam.screen_to_world(Point::new(100.0, 50.0), origin());
    assert!(point_approx_eq(world, Point::new(0.0, 0.0)));
}

#[test]
fn screen_to_world_with_pan_and_zoom() {
    let cam = Camera { pan_x: 20.0, pan_y: 10.0, zoom: 2.0, view_rotation_deg: 0.0 };
    let world = cam.screen_to_world(Point::new(20.0, 10.0), origin());
    assert!(point_approx_eq(world, Point::new(0.0, 0.0)));
}

#[test]
fn screen_to_world_negative_coords() {
    let cam = Camera { pan_x: 0.0, pan_y: 0.0, zoom: 1.0, view_rotation_deg: 0.0 };
    let world = cam.screen_to_world(Point::new(-10.0, -20.0), origin());
    assert!(point_approx_eq(world, Point::new(-10.0, -20.0)));
}

#[test]
fn screen_to_world_origin() {
    let cam = Camera { pan_x: 50.0, pan_y: 30.0, zoom: 2.0, view_rotation_deg: 0.0 };
    let world = cam.screen_to_world(Point::new(0.0, 0.0), origin());
...
fn world_to_screen_identity() {
    let cam = Camera::default();
    let screen = cam.world_to_screen(Point::new(50.0, 75.0), origin());
    assert!(point_approx_eq(screen, Point::new(50.0, 75.0)));
}

#[test]
fn world_to_screen_with_zoom() {
    let cam = Camera { pan_x: 0.0, pan_y: 0.0, zoom: 2.0, view_rotation_deg: 0.0 };
    let screen = cam.world_to_screen(Point::new(10.0, 20.0), origin());
...
fn world_to_screen_with_pan() {
    let cam = Camera { pan_x: 100.0, pan_y: 50.0, zoom: 1.0, view_rotation_deg: 0.0 };
    let screen = cam.world_to_screen(Point::new(0.0, 0.0), origin());
...
fn world_to_screen_with_pan_and_zoom() {
    let cam = Camera { pan_x: 20.0, pan_y: 10.0, zoom: 3.0, view_rotation_deg: 0.0 };
    let screen = cam.world_to_screen(Point::new(5.0, 5.0), origin());
...
fn round_trip_identity() {
    ...
    let screen = cam.world_to_screen(world, origin());
    let back = cam.screen_to_world(screen, origin());
...
fn round_trip_fractional_zoom() {
    let cam = Camera { pan_x: 13.7, pan_y: -42.3, zoom: 0.75, view_rotation_deg: 0.0 };
    let world = Point::new(333.3, -999.9);
    let back = cam.screen_to_world(cam.world_to_screen(world, origin()), origin());
    assert!(point_approx_eq(world, back));
}

#[test]
fn round_trip_screen_first() {
    let cam = Camera { pan_x: 10.0, pan_y: 20.0, zoom: 1.5, view_rotation_deg: 0.0 };
...
#[test]
fn round_trip_cardinal_rotations() {
    let center = Point::new(640.0, 360.0);
    for deg in [0.0, 90.0, 180.0, 270.0] {
        let cam = Camera { pan_x: 18.5, pan_y: -9.25, zoom: 1.8, view_rotation_deg: deg };
        let world = Point::new(123.4, -56.7);
        let screen = cam.world_to_screen(world, center);
...
#[test]
fn round_trip_fractional_rotation_with_pan_and_zoom() {
    let cam = Camera { pan_x: -245.0, pan_y: 112.0, zoom: 2.35, view_rotation_deg: 33.3 };
    let center = Point::new(512.0, 384.0);
    let world = Point::new(-88.25, 401.75);
    let screen = cam.world_to_screen(world, center);
    let back = cam.screen_to_world(screen, center);
    assert!(point_approx_eq(world, back));
}
...
fn screen_dist_to_world_fractional_zoom() {
    let cam = Camera { pan_x: 0.0, pan_y: 0.0, zoom: 0.5, view_rotation_deg: 0.0 };
    assert!(approx_eq(cam.screen_dist_to_world(10.0), 20.0));
}

#[test]
fn screen_dist_to_world_zero() {
    let cam = Camera { pan_x: 0.0, pan_y: 0.0, zoom: 3.0, view_rotation_deg: 0.0 };
    assert!(approx_eq(cam.screen_dist_to_world(0.0), 0.0));
}

#[test]
fn screen_dist_to_world_ignores_pan() {
    let cam = Camera { pan_x: 999.0, pan_y: -999.0, zoom: 4.0, view_rotation_deg: 0.0 };
